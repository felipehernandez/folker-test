...
class PubSubStagePublishAction(PubSubStageAction):
    method: PubSubMethod
    ...
    def execute(self, logger: TestLogger, context: Context) -> Context:
        ...
        self.publisher = (
            PublisherClient(channel=grpc.insecure_channel(target=self.host))
            if self.host
        ...
        self._log_debug(
            logger, topic=topic_path, attributes=attributes, message=str(self.message)
        )
...
class PubSubStageSubscribeAction(PubSubStageAction):
    method: PubSubMethod
    ...
    def execute(self, logger: TestLogger, context: Context) -> Context:
        ...
        self.subscriber = (
            SubscriberClient(channel=grpc.insecure_channel(target=self.host))
            if self.host
        ...
        for message in response.received_messages:
            ...
            context.save_on_stage(
                "message_content", message.message.data.decode("UTF-8")
            )

            if self.ack:
                self.subscriber.acknowledge(
                    subscription=subscription_path, ack_ids=[message.ack_id]
                )

        return context


class PubSubStageTopicsAction(PubSubStageAction):
    method: PubSubMethod
...
class PubSubStageSubscriptionsAction(PubSubStageAction):
    method: PubSubMethod
