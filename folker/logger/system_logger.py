...
class SystemLogger(ABC):
    ...
    def loading_proto_files(self):
        if self.debug:
            ...
            self.report.append(
                LogEntry(type=LogEntryType.SYSTEM_DEBUG, text='Proto files : generating sources'))
        self._log()

    def loading_file_skipped(self, filename):
        if self.trace:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_TRACE_SKIPPED, text=f"{'<SKIP>':8}{filename}"))

    def loading_file_ok(self, filename):
        if self.trace:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_TRACE_OK, text=f"{'<OK>':8}{filename}"))

    def loading_file_error(self, filename, e):
        if self.trace:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_TRACE_FAIL, text=f"{'<ERROR>':8}{filename} - {e}"))
    ...
    def system_setup_completed(self):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_INFO, text=self.DELIMITER_1))
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_INFO, text='SYSTEM SETUP : completed'))
        self._log()
    ...
    def execution_setup_start(self):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_INFO, text=self.DELIMITER_0))
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_INFO, text='Execution SETUP : start'))
        self._log()

    def loading_profile_files(self):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_INFO, text=self.DELIMITER_1))
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_DEBUG, text='Profile files : loading'))
        self._log()

    def loaded_profile(self, profile_name):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_DEBUG, text='Loaded profile: ', end=''))
    ...
    def loading_template_files(self):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_INFO, text=self.DELIMITER_1))
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_DEBUG, text='Template files : loading'))
        self._log()

    def loaded_template(self, template_id):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_DEBUG, text='Loaded template: ', end=''))
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_TRACE_OK, text=template_id))
        self._log()

    def loaded_template_stage(self, stage_id):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_DEBUG, text='\tStage: ', end=''))
    ...
    def loading_test_files(self):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_INFO, text=self.DELIMITER_1))
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_DEBUG, text='Test files : loading'))
        self._log()

    def filtering_tests(self):
        if self.debug:
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_INFO, text=self.DELIMITER_1))
            self.report.append(LogEntry(type=LogEntryType.SYSTEM_DEBUG, text='Test files : filtering'))
        self._log()
        ...
        if self.debug:
            self.report.append(
                LogEntry(type=LogEntryType.SYSTEM_TRACE_WARN,
                         text=f"{'<SKIP>':10}{test_name} - Skip tag matching : {sorted(matching_skip_tags)}"))
        self._log()
        ...
        if self.debug:
            self.report.append(
                LogEntry(type=LogEntryType.SYSTEM_TRACE_OK,
                         text=f"{'<EXECUTE>':10}{test_name} - Execute tag matching : {sorted(matching_execute_tags)}"))
        self._log()

    def test_filter_out_execution_tags(self, test_name: str, not_matching_tags: Set[str]):
        if self.debug:
            self.report.append(
                LogEntry(type=LogEntryType.SYSTEM_TRACE_WARN,
                         text=f"{'<SKIP>':10}{test_name} - Missing execution tags : {sorted(not_matching_tags)}"))
        self._log()

    def test_filter_tags(self, test_name: str):
        if self.debug:
            self.report.append(
                LogEntry(type=LogEntryType.SYSTEM_TRACE_OK,
                         text=f"{'<EXECUTE>':10}{test_name} - No tags restriction defined"))
        self._log()
    ...
    def test_filter_in_skip_tags(self, test_name: str):
        if self.debug:
            self.report.append(
                LogEntry(type=LogEntryType.SYSTEM_TRACE_OK,
                         text=f"{'<EXECUTE>':10}{test_name} - No skip tag matching"))
        self._log()
    ...
    def _log_report_failures(self, failures):
        self.report.append(LogEntry(type=LogEntryType.REPORT_FAILURE, text=f'Failures: ({len(failures)})'))
        if self.debug:
            for failure in failures:
                self.report.append(LogEntry(type=LogEntryType.REPORT_FAILURE_DEBUG, text=f'\t{failure}'))

    def _log_report_successes(self, successes):
        self.report.append(LogEntry(type=LogEntryType.REPORT_SUCCESS, text=f'Success: ({len(successes)})'))
        if self.debug:
            for success in successes:
                self.report.append(LogEntry(type=LogEntryType.REPORT_SUCCESS_DEBUG, text=f'\t{success}'))
