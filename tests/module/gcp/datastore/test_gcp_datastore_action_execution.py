...
from folker.module.gcp.datastore.action import DatastoreStageAction, DatastoreStageBulkDeleteAction, DatastoreStageDeleteAction, DatastoreStageGetAction, DatastoreStagePutAction, DatastoreStageQueryAction


@pytest.mark.action_gcp_datastore
def test_gcp_datastore_put(mocker, plain_console_test_logger_on_trace):
    entity_data = {}
    ...
    datastore_client = mocker.patch(
        "folker.module.gcp.datastore.action.Client", name="Client"
    )
    ...
    action.execute(logger=plain_console_test_logger_on_trace, context=Context())
...
@pytest.mark.action_gcp_datastore
def test_gcp_datastore_get_existing(mocker, plain_console_test_logger_on_trace):
...
@pytest.mark.action_gcp_datastore
def test_gcp_datastore_get_not_existing(mocker, plain_console_test_logger_on_trace):
    action = DatastoreStageGetAction(
        method="GET",
        host="aHost",
        project="aProject",
...
@pytest.mark.action_gcp_datastore
def test_gcp_datastore_delete(mocker, plain_console_test_logger_on_trace):
...
@pytest.mark.action_gcp_datastore
def test_gcp_datastore_query(mocker, plain_console_test_logger_on_trace):
    action = DatastoreStageQueryAction(
        method="QUERY", host="aHost", project="aProject", key={"kind": "KEY_KIND"}
    )
...
@pytest.mark.action_gcp_datastore
def test_gcp_datastore_bulkdelete(mocker, plain_console_test_logger_on_trace):
    action = DatastoreStageBulkDeleteAction(
        method="BULK_DELETE",
        host="aHost",
        project="aProject",
    ...
    datastore_client.key.assert_has_calls(
        [call("KEY_KIND", "id1"), call("KEY_KIND", "id2")]
    )
