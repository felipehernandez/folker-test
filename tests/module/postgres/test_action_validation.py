...
from folker.module.postgres.action import PostgresStageAction, PostgresMethod, PostgresStageSelectAction


@pytest.mark.action_correctness
@pytest.mark.action_postgres
class TestPostgresStageActionValidation:
...
@pytest.mark.action_correctness
@pytest.mark.action_postgres
class TestPostgresStageSelectActionValidation:
    def test_correct(self):
        action = PostgresStageSelectAction(
            method=PostgresMethod.SELECT.name,
            host="a_host",
            port="5432",
            user="a_user",
            password="a_password",
