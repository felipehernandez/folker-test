...
class TestRabbitMQActionEnrichment:
    def test_enrich_message(self):
        original = RabbitMQStageAction(
            method=RabbitMQMethod.PUBLISH.name,
            host="a_host",
            exchange="an_exchange",
            message="Old message",
        )
        enrichment = RabbitMQStageAction(
            method=RabbitMQMethod.PUBLISH.name, message="New message"
        )
    ...
    def test_enrich_ack(self):
        ...
        enrichment = RabbitMQStageAction(method=RabbitMQMethod.SUBSCRIBE.name, ack=True)
