...
from folker.module.rabbitmq.action import (RabbitMQMethod,
...
class TestRabbitMQPublishAction:
    action: RabbitMQStagePublishAction

    @pytest.fixture(autouse=True)
    def setup(self):
        self.action = RabbitMQStagePublishAction()
        yield
    ...
    def test_simple_publish_execution(
        ...
        ConnectionParameters.assert_called_once_with(
            host="a-host", virtual_host="a-virtual-host"
        )
        ...
        BlockingConnection.return_value.channel.return_value.basic_publish.assert_called_with(
            exchange="an-exchange", routing_key="", body="a-message".encode()
        )
...
@pytest.mark.action_rabbitmq
class TestRabbitMQSubscribeAction:
    action: RabbitMQStageSubscribeAction

    @pytest.fixture(autouse=True)
    def setup(self):
        self.action = RabbitMQStageSubscribeAction()
        yield

    @patch("folker.module.rabbitmq.action.pika.ConnectionParameters")
    @patch("folker.module.rabbitmq.action.pika.BlockingConnection")
    def test_simple_subscription_execution(
            self,
            BlockingConnection,
            ConnectionParameters,
            plain_console_test_logger_on_trace,
        ...
        BlockingConnection.return_value.channel.return_value.basic_get.return_value = (
            None,
            None,
            "a-message".encode(),
        )
...
@pytest.mark.action_rabbitmq
class TestRabbitMQCountAction:
    action: RabbitMQStageCountAction

    @pytest.fixture(autouse=True)
    def setup(self):
        self.action = RabbitMQStageCountAction()
        yield
    ...
    def test_simple_count(
        ...
        BlockingConnection.return_value.channel.return_value.queue_declare.return_value = (
            queue_declaration
        )
        ...
        BlockingConnection.return_value.channel.return_value.queue_declare.assert_called_with(
            queue="a-queue", passive=True
        )
...
@pytest.mark.action_rabbitmq
class TestRabbitMQClearAction:
    action: RabbitMQStageClearAction

    @pytest.fixture(autouse=True)
    def setup(self):
        self.action = RabbitMQStageClearAction()
        yield
    ...
    def test_simple_clear(
        ...
        BlockingConnection.return_value.channel.return_value.queue_purge.return_value = (
            queue_purge_result
        )
        ...
        BlockingConnection.return_value.channel.return_value.queue_purge.assert_called_with(
            queue="a-queue"
        )
