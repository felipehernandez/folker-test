...
class TestKafkaPublishAction:
    action: KafkaStagePublishAction

    @pytest.fixture(autouse=True)
    def setup(self):
        self.action = KafkaStagePublishAction()
        yield

    @patch("folker.module.kafka.action.KafkaProducer")
    def test_simple_publish_execution(
        self, KafkaProducer, plain_console_test_logger_on_trace
    ):
        ...
        KafkaProducer.return_value.send.assert_called_with(
            topic="a-topic", key="a-key".encode(), value=None, headers=[]
        )
...
@pytest.mark.action_kafka
class TestKafkaStageSubscribeAction:
    action: KafkaStageSubscribeAction

    @pytest.fixture(autouse=True)
    def setup(self):
        self.action = KafkaStageSubscribeAction()
        yield

    @patch("folker.module.kafka.action.KafkaConsumer")
    def test_simple_subscription_execution(
        self, KafkaConsumer, plain_console_test_logger_on_trace
    ):
